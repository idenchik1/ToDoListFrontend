        ...
        if (changedTask.length === 2) {
            ...
            if (changedTask[0] === "") {
                if (changedTask[1].length >= 4) {
                    fetch('https://localhost/api/user/CreateTask/' + id, {
                        method: 'post',
                        headers: {
                            'Authorization': 'Bearer ' + token,
            ...
            if (changedTask[1].length === 0) {
                fetch('https://localhost/api/user/DeleteTask/' + changedTask[0], {
                    method: 'delete',
                    headers: {
                        'Authorization': 'Bearer ' + token
                    }
            ...
            if (changedTask[1].length >= 4 && changedTask[1].length <= 256) {
                fetch('https://localhost/api/user/EditTask/' + changedTask[0], {
                    method: 'put',
                    headers: {
                        'Authorization': 'Bearer ' + token,
                        'Content-Type': 'application/json'
